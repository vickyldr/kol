...
- unit_price: 单价（每条视频的价格，只要纯数字，不要带币种符号/文字）
- account_name: 收款信息里的账户名称/收款人姓名
- payment_method: 收款方式（如 PayPal、银行转账等）
- currency: 币种，统一输出 ISO 4217 标准代码（大写三字母）。
           例：美元/美金/$→USD；新台币/台币/NT$→TWD；欧元/Euro/€→EUR；
           日元/円→JPY；韩元→KRW；土耳其里拉/TL→TRY；泰铢/Thai/Baht→THB；
           人民币→CNY；英镑→GBP。其它币种同理转成对应 ISO 代码。
...
- account_name: 收款账户名称
- currency: 币种，统一输出 ISO 4217 标准代码（大写三字母）。
           例：美元/美金→USD；新台币/台币→TWD；欧元→EUR；日元→JPY；韩元→KRW；
           土耳其里拉→TRY；泰铢→THB；人民币→CNY。其它币种同理转成对应 ISO 代码。
- amount: 审批金额（只要纯数字，不要带币种符号/文字、不要千分位逗号）
- platform_count: 平台数（整数；读不出就填实际投放平台的去重个数）
- collab_video_count: 填写的合作视频数量
- video_list: 视频清单（每条视频链接/标识放进数组）
- platforms: 视频清单里每条的「投放平台」，原样放进数组（如 ["Instagram","TikTok","Youtube"]）
- online_type: 「上线类型」字段原文（如 KOL上线 / 社媒采买 / SEO上线 / 非KOL上线）
- is_prepayment: 是否预付款/分期/尾款（true/false）。判断依据有两处，任一命中即 true：
    (1) 审批流程字段是「预付款流程」；
    (2) 备注里出现「预付款 / 已预支付 / 分期 / 尾款 / 支付剩余」等字样
        （有的单审批流程写"现付款"，但备注才点明是预付尾款，必须按备注判）
- is_non_kol: 是否非 KOL 上线（true/false）；上线类型不是「KOL上线」就填 true
